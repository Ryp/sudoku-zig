...
pub fn execute_main_loop(io: std.Io, allocator: std.mem.Allocator) !void {
    const board_config = known_boards.easy;
    const extent = comptime board_config.rules.type.extent();

    var game = try game_state.State.init(io, allocator, board_config.rules, board_config.start_string);
...
fn create_font_textures_and_aabbs(allocator: std.mem.Allocator, vram_allocator: std.mem.Allocator, ttf: TrueType, scale: f32, board_extent: u32) !struct { []Texture, []SDL_FRect } {
    ...
    var glyph_indices_full: [board.MaxExtent]TrueType.GlyphIndex = undefined;
...
fn draw_osk(extent: comptime_int, board_state: board.Board, sdl_context: PspContext(extent), ui_state: UIState) void {
    psp.sceGuDisable(.Blend);
    psp.sceGuDisable(.Texture2D);
    ...
    for (board_state.numbers_const()) |number_opt| {
        if (number_opt) |number| {
            counts[number] += 1;
        }
    }
...
fn draw_solver_technique_overlay(extent: comptime_int, board_state: board.Board, sdl_context: PspContext(extent), technique: solver_logical.Technique) void {
    switch (technique) {
...
fn draw_validation_error(extent: comptime_int, board_state: board.Board, sdl_context: PspContext(extent), validation_error: validator.Error) void {
...
fn draw_sudoku_box_regions(extent: comptime_int, board_state: board.Board, sdl_context: PspContext(extent)) void {
